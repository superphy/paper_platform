...
% 3. how we solved it
In this study, we present an update to the SuperPhy platform, called Spfy. Spfy enables large-scale genome processing in real-time and uniquely decomposses results into graph data structures for further comparative genomics. Unlike traditional approaches which store results in separate data files or in a relational database, the graph data structures are then integrated into a knowledge graph identifying relationships between all genomes sequenced in the past, present, and future. We present the use of graph databases as a solution to storing genome analysis results in a meaningful manner, and to allow retrospective comparisons as more genomes are sequenced or populations change.
