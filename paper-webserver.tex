...
% 2. Why solving it is important (for Public health / research)
WGS is becoming the norm for bacterial surveillance, outbreak tracing, and research \cite{ronholm2016navigating}. While pipelines exist \cite{joensen2014real} for running a variety of analyses on WGS data, 
In order to  Existing annotation and analysis software for WGS data represent units of compuatation; we can optimize WGS pipelines if these results are stored to avoid recomputation, and extend results by viewing them in the context of other analysis modules and genomes.
 these computations can be 
...
% 3. how we solved it
In this study, we present an update to the SuperPhy platform, called Spfy. Spfy enables large-scale genome processing in real-time and uniquely decomposses results into graph datastructures for further comparative genomics. Unlike traditional approaches which store results in separate data files or in a relational database, the graph datastructures are then integrated into a knowledge graph identifying relationships between all genomes sequenced in the past, present, and future. We present the use of graph databases as a solution to storing genome analysis results in a meaningful manner, and to allow retrospective comparisons as more genomes are sequenced or populations change.

{I'm not sure if we should include that the same graph database can be expanded to cover different spcies}
...
The entire Spfy update is packaged in Docker \url{https://www.docker.com/} containers and connected together using Docker Compose \url{https://docs.docker.com/compose/}.
