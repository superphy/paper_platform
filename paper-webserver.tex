...
As food-sourcing is globalized, consumers are at a greater risk for bacterial infections in an environment that is more complicated to track than ever before \cite{kozak2013foodborne}.
\textit{Salmonella enterica} and \textit{Escherichia coli} are amongst the two most common bacetrial infections \cite{kozak2013foodborne} originating from produce.
Foodborne illnesses translate into actual costs: for example, in Canada alone an estimated 4000 hospitalizations and 105 deaths per year are thought to be the result of various pathogens \cite{thomas2015estimates}.
...
Software such as VirulenceFinder \cite{joensen2014real} compares a pre-collected set of reference sequences encompassing known virulence factor gene sequences against a WGS result.
...
% potential of WGS
WGS has the potential to expand on traditional methods, due to the inclusion of the entire genome in WGS-based analyses; in effect, this lends WGS-based methods better resolution and context in evaluating the spread of outbreaks \cite{ronholm2016navigating}.
As an example, instead of examining selected single-nucleotide polymorphisms (SNPs) in a subset of housekeeping genes, as is the case for laboratory-based MLST, newer methods such as core genome MLST (cgMLST) attempt to cover every common gene.
...
% Drawbacks - genetics
However, these WGS-base methods still examine the underlying genetics, not the expressed phenotype.
The presence of a particular gene sequence does not necessitate expression.
Correspondingly, a greater number of target sequences are identified which complicates risk assessments \cite{ronholm2016navigating} and laboratory based phenotypic tests are still relatively inexpensive \cite(CITE ROUTINE USE - KOSER).
The effectiveness of WGS-based methods is also reliant on the accuracy and breadth of reference datasets.

% drawbacks - big data
Traditionaly, bioinformatics software was developed \textit{ad hoc}, without employing software engineering principles \cite(TRENDS - BREVEREN).
Such approaches were often script-based, with custom data formats, and only suitable for small collections of data \cite(TRENDS - BREVEREN).
As WGS-technology expanded the availability of data, systems engineering principles became important as code failure could quickly disrupt entire bioinformatics pipelines \cite(BIOLOGICAL DATA SCIENCES - SCHATZ).
The field also lags behind other big data industries: bioinformatics software typically struggles to perform computations in real-time, stores data in inflexible databases, doesn't support live visualizations \cite(TRENDS - BREVEREN), and lacks a common application programming interface (API) linking resources \cite(REVIEW - SWAMINATHAN).
...
For genotypic, WGS-based approaches to be truly compliment laboratory-based phenotypic tests, analyses software must be faster or demonstrate a reasonable link between their genotype and phenotype \cite(CITE ROUTINE USE - KOSER).
Antimicrobial resistance testing, which is a major component of public health monitoring efforts, typically takes 18-24 hours in a reference laboratory \cite(CITE ROUTINE USE - KOSER).
Results should also be stored in a tiered structure, with rapid access available for retrieval and visualization while maintaining long-term data storage \cite(BIOLOGICAL DATA SCIENCES - SCHATZ).
Such an approach would enable developers to address the current limitations \cite(TRENDS - BREVEREN) of bioinformatics developers.
...
% this study
Here we present Spfy, a complete rewrite of Superphy which expands on supported features to also provide long-term storage of results in a graph database, one of the emerging \cite(TRENDS - BREVEREN) database types being used for biological data.
We also containerize of services using Docker, a modern approach found in many web servers and proposed to mitigate the complexities of bioinformatic pipelines \cite{di2015impact}.
...
Furthermore, Spfy takes a modular approach to tool integration and distrib,utes analyses over task queues, thereby allowing modules to run in parallel and addresses code failures.
