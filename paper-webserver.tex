...
The cache allows Spfy to more efficiently check for duplicate files in Blazegraph, than would be possible through an exaustive search of the graph structure.
...
The VM is running Linux with CentOS.
...
On analysis tasks, Spfy runs all included modules in 130 seconds on average, per file \ref{fig:spfy_performance_indivs}.
Spfy can also queue up batches of files for analysis, decreasing the average runtime to an average of 54 seconds per file due to parallelization of analysis runs \ref{fig:spfy_performance_batches}.
In total, its took 45 minutes to analyze 50 files and 89 minutes to analyze 100 files.
